...
namespace RAW
{
    public partial class Seller_Cancel_Job : Form
    {
        ...
        Seller_cancelJob_Panel[] scjp = new Seller_cancelJob_Panel[50];
        [DllImport("user32.DLL", EntryPoint = "ReleaseCapture")]
        private extern static void ReleaseCapture();
        [DllImport("user32.DLL", EntryPoint = "SendMessage")]
        private extern static void SendMessage(System.IntPtr one, int two, int three, int four);
        public Seller_Cancel_Job()
        {
            InitializeComponent();
        }
        ...
        private void Seller_Cancel_Job_Load(object sender, EventArgs e)
        {
            customizeSubMenu();
...

            {
                ...
                String query = "SELECT * FROM PROGRESS_JOB WHERE SELLER_NAME= @user;";
                SqlCommand cmd = new SqlCommand(query, con);
                cmd.Parameters.AddWithValue("@user", Seller_Info.USER_NAME);
               // cmd.Parameters.AddWithValue("@jstatus", "Pending");
                con.Open();
                    ...
                    while (sda.Read())
                    {
                        ...
                        if (sda1.HasRows == true)
                        {
                            ...
                            while (sda1.Read())
                            {
                                ...
                                String bdet = (sda1["JOB_DETAILS"].ToString());
                                String bprice = (sda1["JOB_PRICE"].ToString());
                                String btime = (sda1["JOB_TIME"].ToString());
                                String stat = (sda1["JOB_STATUS"].ToString());
                             


                                scjp[i] = new Seller_cancelJob_Panel(image, bname, bpost, bdet, bprice, btime, sname);

                                SellercancelancelJobPanel.Controls.Add(scjp[i]);
                                scjp[i].Location = new System.Drawing.Point(x, y);
                                scjp[i].Visible = true;
                                scjp[i].BringToFront();
