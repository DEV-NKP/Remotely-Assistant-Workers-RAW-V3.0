...
namespace RAW
{
    public partial class Seller_Submitted_Job : Form
    {
        int viewp = 1;

        Seller_SubmittedJob_Panel[] ssjp = new Seller_SubmittedJob_Panel[50];
        ...
        private void Seller_Submitted_Job_Load(object sender, EventArgs e)
        {
            customizeSubMenu();

            {
                ...
                cmd.Parameters.AddWithValue("@trans", "NO");
                con.Open();
                    ...
                    while (sda.Read())
                    {
                        ...
                        if (sda1.HasRows == true)
                        {
                            ...
                            while (sda1.Read())
                            {
                                ...
                                ssjp[i] = new Seller_SubmittedJob_Panel(image, bname, bpost, slink, bprice, btime, sname);

                                sellerSubmittedJobPanel.Controls.Add(ssjp[i]);
                                ssjp[i].Location = new System.Drawing.Point(x, y);
                                ssjp[i].Visible = true;
                                ssjp[i].BringToFront();
